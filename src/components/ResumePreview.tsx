import type { ResumeData } from '../types/resume'
import '../styles/resume.css'

interface Props {
  data: ResumeData
}

const icons = {
  location: (
    <svg className="contact-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0118 0z" /><circle cx="12" cy="10" r="3" />
    </svg>
  ),
  email: (
    <svg className="contact-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><polyline points="22,6 12,13 2,6" />
    </svg>
  ),
  phone: (
    <svg className="contact-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.8a19.79 19.79 0 01-3.07-8.7A2 2 0 012.18 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 7.09a16 16 0 006 6l.56-.56a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 14.92z" />
    </svg>
  ),
  globe: (
    <svg className="contact-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="12" cy="12" r="10" /><line x1="2" y1="12" x2="22" y2="12" /><path d="M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z" />
    </svg>
  ),
  linkedin: (
    <svg className="contact-icon" viewBox="0 0 24 24" fill="currentColor">
      <path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6zM2 9h4v12H2z" /><circle cx="4" cy="4" r="2" />
    </svg>
  ),
  github: (
    <svg className="contact-icon" viewBox="0 0 24 24" fill="currentColor">
      <path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 00-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0020 4.77 5.07 5.07 0 0019.91 1S18.73.65 16 2.48a13.38 13.38 0 00-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 005 4.77a5.44 5.44 0 00-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 009 18.13V22" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none" />
    </svg>
  ),
  relocate: (
    <svg className="contact-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 2L2 7l10 5 10-5-10-5z" /><path d="M2 17l10 5 10-5" /><path d="M2 12l10 5 10-5" />
    </svg>
  ),
}

function ensureHttp(url: string) {
  if (!url) return '#'
  return url.startsWith('http') ? url : `https://${url}`
}

function SidebarHeading({ children }: { children: React.ReactNode }) {
  return <div className="sidebar-heading">{children}</div>
}

function MainHeading({ children }: { children: React.ReactNode }) {
  return (
    <div className="main-heading">
      <span className="main-heading-bar" />
      {children}
    </div>
  )
}

export default function ResumePreview({ data }: Props) {
  return (
    <div className="resume-wrapper">
      <div className="resume-container">

        {/* ── Sidebar ── */}
        <div className="resume-sidebar">
          <div>
            <SidebarHeading>Contact</SidebarHeading>
            {data.location && (
              <div className="contact-item">
                {icons.location}
                <span className="contact-text">{data.location}</span>
              </div>
            )}
            {data.openToRelocate && (
              <div className="contact-item">
                {icons.relocate}
                <span className="contact-text">Open to relocation · {data.openToRelocate}</span>
              </div>
            )}
            {data.email && (
              <div className="contact-item">
                {icons.email}
                <a className="contact-link" href={`mailto:${data.email}`}>{data.email}</a>
              </div>
            )}
            {data.phone && (
              <div className="contact-item">
                {icons.phone}
                <a className="contact-link" href={`tel:${data.phone}`}>{data.phone}</a>
              </div>
            )}
            {data.website && (
              <div className="contact-item">
                {icons.globe}
                <a className="contact-link" href={ensureHttp(data.website)} target="_blank" rel="noreferrer">{data.website}</a>
              </div>
            )}
            {data.linkedin && (
              <div className="contact-item">
                {icons.linkedin}
                <a className="contact-link" href={ensureHttp(data.linkedin)} target="_blank" rel="noreferrer">{data.linkedin}</a>
              </div>
            )}
            {data.github && (
              <div className="contact-item">
                {icons.github}
                <a className="contact-link" href={ensureHttp(data.github)} target="_blank" rel="noreferrer">{data.github}</a>
              </div>
            )}
          </div>

          {data.skills.length > 0 && (
            <div>
              <SidebarHeading>Skills</SidebarHeading>
              <div className="skills-wrap">
                {data.skills.map(skill => (
                  <span key={skill} className="skill-tag">{skill}</span>
                ))}
              </div>
            </div>
          )}

          {data.education.length > 0 && (
            <div>
              <SidebarHeading>Education</SidebarHeading>
              {data.education.map(edu => (
                <div key={edu.id} className="edu-item">
                  <div className="edu-school">{edu.school}</div>
                  {(edu.degree || edu.field) && (
                    <div className="edu-degree">
                      {[edu.degree, edu.field].filter(Boolean).join(', ')}
                    </div>
                  )}
                  {(edu.startDate || edu.endDate) && (
                    <div className="edu-dates">
                      {edu.startDate}{edu.endDate ? ` – ${edu.endDate}` : ''}
                    </div>
                  )}
                </div>
              ))}
            </div>
          )}
        </div>

        {/* ── Main ── */}
        <div className="resume-main">
          <div className="resume-header">
            <div className="resume-name">{data.name || 'Your Name'}</div>
            <div className="resume-job-title">{data.title || 'Your Title'}</div>
            <div className="main-divider" />
          </div>

          {data.summary && (
            <div className="main-section">
              <MainHeading>About Me</MainHeading>
              <p className="summary-text">{data.summary}</p>
            </div>
          )}

          {data.workExperience.length > 0 && (
            <div className="main-section">
              <MainHeading>Experience</MainHeading>
              {data.workExperience.map(exp => (
                <div key={exp.id} className="exp-item">
                  <div className="exp-company-line">
                    <span className="exp-company">{exp.company}</span>
                    {exp.location && <span className="exp-location">{exp.location}</span>}
                  </div>
                  {exp.roles.map(role => (
                    <div key={role.id} className="exp-role-entry">
                      <div className="exp-meta">
                        <span className="exp-role">{role.role}</span>
                        <span className="exp-dates">
                          {role.startDate}
                          {(role.startDate || role.endDate || role.current) && ' – '}
                          {role.current ? 'Present' : role.endDate}
                        </span>
                      </div>
                      {role.description && (
                        <div className="exp-description">
                          {role.description.split('\n').filter(l => l.trim()).map((line, i) => (
                            <div key={i} className="exp-bullet">
                              {line.replace(/^[•\-]\s*/, '')}
                            </div>
                          ))}
                        </div>
                      )}
                      {role.stack?.length > 0 && (
                        <div className="exp-stack">
                          {role.stack.map(tech => (
                            <span key={tech} className="exp-stack-tag">{tech}</span>
                          ))}
                        </div>
                      )}
                    </div>
                  ))}
                </div>
              ))}
            </div>
          )}
        </div>
      </div>
    </div>
  )
}
